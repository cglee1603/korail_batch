...
def run_batch(excel_path: str = None):
    """배치 작업 실행"""
    start_time = datetime.now()
    logger.info(f"\n배치 작업 시작 시간: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    
    try:
        processor = BatchProcessor(excel_path=excel_path)
        processor.process()
...
def setup_schedule(excel_path: str = None):
    """스케줄 설정"""
    ...
    if schedule_type == 'time':
        # 특정 시간에 실행
        schedule.every().day.at(value).do(run_batch, excel_path=excel_path)
        logger.info(f"스케줄 설정 완료: 매일 {value}에 실행")
    
    elif schedule_type == 'multiple':
        # 여러 시간에 실행
        for time_str in value:
            schedule.every().day.at(time_str).do(run_batch, excel_path=excel_path)
        logger.info(f"스케줄 설정 완료: 매일 {', '.join(value)}에 실행")
    
    elif schedule_type == 'interval':
        # 주기적 실행
        schedule.every(value).seconds.do(run_batch, excel_path=excel_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='RAGFlow Plus 배치 프로그램 - 엑셀에서 파일을 추출하여 지식베이스에 업로드'
    )
    ...
    # 스케줄 설정
    has_schedule = setup_schedule(excel_path)
